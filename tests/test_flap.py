...
class TestAuctionKeeperFlapper(TransactionIgnoringTest):
    def setup_method(self):
        ...
        self.flapper.approve(
            self.mcd.mkr.address, directly(from_address=self.other_address)
        )

        self.keeper = AuctionKeeper(
            args=args(
    ...
    def test_should_start_a_new_model_and_provide_it_with_info_on_auction_kick(
        ...
        model_factory.create_model.assert_called_once_with(
            Parameters(
                flipper=None, flapper=self.flapper.address, flopper=None, id=kick
            )
        )
        # and
    ...
    def test_should_provide_model_with_updated_info_after_somebody_else_bids(
        ...
        assert self.flapper.tend(kick, auction.lot, Wad.from_number(40)).transact(
    ...
    def test_should_not_instantiate_model_if_auction_is_dealt(self, kick):
        ...
        # and
        auction = self.flapper.bids(kick)
        self.flapper.tend(kick, auction.lot, Wad.from_number(40)).transact(
            from_address=self.other_address
        )
        # and
        time_travel_by(self.web3, self.flapper.ttl() + 1)
    ...
    def test_should_make_initial_bid(self):
        ...
        # then
        auction = self.flapper.bids(kick)
        assert round(Wad(auction.lot) / auction.bid, 2) == round(
            Wad.from_number(10.0), 2
        )
    ...
    def test_should_bid_even_if_there_is_already_a_bidder(self, kick):
        ...
        # and
        auction = self.flapper.bids(kick)
        assert self.flapper.tend(kick, auction.lot, Wad.from_number(16)).transact(
            from_address=self.other_address
        )
    ...
    def test_should_increase_gas_price_of_pending_transactions_if_model_increases_gas_price(
        ...
        assert (
            self.web3.eth.getBlock("latest", full_transactions=True)
            .transactions[0]
            .gasPrice
            == 15
        )
    ...
    def test_should_replace_pending_transactions_if_model_raises_bid_and_increases_gas_price(
        ...
        wait_for_other_threads()
        # then
        assert round(self.flapper.bids(kick).bid, 2) == round(
            Wad(lot / Rad.from_number(8.0)), 2
        )
        assert (
            self.web3.eth.getBlock("latest", full_transactions=True)
            .transactions[0]
    ...
    def test_should_not_bid_on_rounding_errors_with_small_amounts(self, kick):
        ...
        # then
        assert (
            self.web3.eth.getTransactionCount(self.keeper_address.address) == tx_count
        )
    ...
    def test_should_not_deal_when_auction_finished_but_somebody_else_won(self, kick):
        ...
        lot = self.flapper.bids(kick).lot
        # and
        assert self.flapper.tend(kick, lot, Wad.from_number(16)).transact(
            from_address=self.other_address
        )
        assert self.flapper.bids(kick).bid == Wad.from_number(16)
    ...
    def test_should_obey_gas_price_provided_by_the_model(self, kick):
        ...
        simulate_model_output(
            model=model, price=Wad.from_number(10.0), gas_price=175000
        )
        # and
        self.keeper.check_all_auctions()
        self.keeper.check_for_bids()
    ...
    def test_should_use_default_gas_price_if_not_provided_by_the_model(self, kick):
        ...
        print(
            f"tx gas price is {self.web3.eth.getBlock('latest', full_transactions=True).transactions[0].gasPrice}, web3.eth.gasPrice is {self.web3.eth.gasPrice}"
        )
    ...
    def teardown_class(cls):
        cls.mcd = mcd(web3())
        cls.liquidate_urn(
            web3(), cls.mcd, c(cls.mcd), gal_address(web3()), our_address(web3())
        )
    ...
    def liquidate_urn(cls, web3, mcd, c, gal_address, our_address):
        ...
        lump = mcd.cat.lump(c.ilk)
        urn = mcd.vat.urn(c.ilk, gal_address)
        bites_required = math.ceil(urn.ink / lump)
        print(f"ink={urn.ink} so {bites_required} bites are required")
        c.flipper.approve(
            mcd.vat.address, approval_function=hope_directly(from_address=our_address)
        )
