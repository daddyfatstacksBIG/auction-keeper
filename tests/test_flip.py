...
class TestAuctionKeeperFlipper(TransactionIgnoringTest):
    ...
    def simulate_model_bid(
        ...
        reserve_dai(
            mcd, c, self.keeper_address, our_bid, extra_collateral=Wad.from_number(2)
        )
    ...
    def test_should_start_a_new_model_and_provide_it_with_info_on_auction_kick(
        ...
        model_factory.create_model.assert_called_once_with(
            Parameters(
                flipper=self.collateral.flipper.address,
                flapper=None,
                flopper=None,
                id=kick,
            )
    ...
    def test_should_provide_model_with_updated_info_after_somebody_else_bids(
        ...
        # when
        flipper.approve(
            flipper.vat(), approval_function=hope_directly(from_address=other_address)
        )
        previous_bid = flipper.bids(kick)
        new_bid_amount = Rad.from_number(80)
        self.tend_with_dai(
            self.mcd, self.collateral, flipper, model.id, other_address, new_bid_amount
        )
        # and
        self.keeper.check_all_auctions()
        wait_for_other_threads()
    ...
    def test_should_terminate_model_if_auction_expired_due_to_ttl_and_somebody_else_won_it(
        ...
        self.tend_with_dai(
            self.mcd, self.collateral, flipper, kick, other_address, new_bid_amount
        )
        # and
        time_travel_by(self.web3, flipper.ttl() + 1)
        # and
    ...
    def test_should_terminate_model_if_auction_is_dealt(self, kick, other_address):
        ...
        # when
        self.tend_with_dai(
            self.mcd, self.collateral, flipper, kick, other_address, Rad.from_number(90)
        )
        # and
        time_travel_by(self.web3, flipper.ttl() + 1)
        # and
    ...
    def test_should_not_instantiate_model_if_auction_is_dealt(
        ...
        flipper = self.collateral.flipper
        # and
        TestAuctionKeeperFlipper.tend_with_dai(
            self.mcd, self.collateral, flipper, kick, other_address, Rad.from_number(90)
        )
        # and
        time_travel_by(self.web3, flipper.ttl() + 1)
    ...
    def test_should_make_initial_bid(self, kick):
        ...
        assert round(auction.bid / Rad(auction.lot), 2) == round(
            Rad.from_number(16.0), 2
        )
    ...
    def test_should_bid_even_if_there_is_already_a_bidder(self, kick, other_address):
        ...
        # and
        self.tend_with_dai(
            self.mcd, self.collateral, flipper, kick, other_address, Rad.from_number(21)
        )
    ...
    def test_should_increase_gas_price_of_pending_transactions_if_model_increases_gas_price(
        ...
        reserve_dai(
            self.mcd, self.collateral, self.keeper_address, bid_price * tend_lot * 2
        )
    ...
    def test_should_replace_pending_transactions_if_model_lowers_bid_and_increases_gas_price(
        ...
        bid_price = Wad.from_number(20.0)
        reserve_dai(
            self.mcd, self.collateral, self.keeper_address, bid_price * tend_lot
        )
    ...
    def test_should_use_default_gas_price_if_not_provided_by_the_model(self, kick):
        ...
        # when
        self.simulate_model_bid(
            self.mcd, self.collateral, model, price=Wad.from_number(16.0)
        )
        # and
        self.keeper.check_all_auctions()
        self.keeper.check_for_bids()
    ...
    def test_should_change_gas_strategy_when_model_output_changes(self, kick):
        ...
        self.simulate_model_bid(
            self.mcd, self.collateral, model=model, price=first_bid, gas_price=2000
        )
        # and
        self.keeper.check_all_auctions()
        self.keeper.check_for_bids()
        ...
        self.simulate_model_bid(
            self.mcd, self.collateral, model=model, price=second_bid
        )
        # and
        self.keeper.check_all_auctions()
        self.keeper.check_for_bids()
