...
web3 = Web3(HTTPProvider(
    endpoint_uri=os.environ['ETH_RPC_URL'], request_kwargs={"timeout": 30}))
...
collateral = mcd.collaterals[str(sys.argv[3])] if len(
    sys.argv) > 2 else mcd.collaterals['ETH-A']
...
def create_risky_vault():
    ...
    if not is_cdp_safe(mcd.vat.ilk(collateral.ilk.name), urn):
        ...
        print(
            f"dust={ilk.dust} osm_price={osm_price} mat={mcd.spotter.mat(ilk)}")
        # To make a (barely) safe urn, I expected to add 10^-8, but for some reason I need to add 10^-7
        normalized_collateral_amount = (Wad(
            ilk.dust) / osm_price * Wad(mcd.spotter.mat(ilk))) + Wad.from_number(0.00000050)
        ...
        create_risky_cdp(mcd, collateral, Wad.from_number(
            normalized_collateral_amount), our_address, True)
...
def handle_returned_collateral():
    ...
    if available_to_generate > Wad(1):
        ...
        assert mcd.dai_adapter.exit(
            our_address, available_to_generate).transact()
