...
collateral = (mcd.collaterals[str(sys.argv[3])]
              if len(sys.argv) > 2 else mcd.collaterals["ETH-A"])
...
def create_risky_vault():
    ...
    if not is_cdp_safe(mcd.vat.ilk(collateral.ilk.name), urn):
        ...
        normalized_collateral_amount = (Wad(ilk.dust) / osm_price * Wad(
            mcd.spotter.mat(ilk))) + Wad.from_number(0.00000050)
        ...
        create_risky_cdp(
            mcd,
            collateral,
            Wad.from_number(normalized_collateral_amount),
            our_address,
            True,
        )
